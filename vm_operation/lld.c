...
/*   Updated: 2020/01/01 21:32:46 by bsabre-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static short	is_invalid_parameters(unsigned char types, short reg_num)
{
	...
	if (((DIR_CODE << 6) | (REG_CODE << 4)) != types && ((IND_CODE << 6) | (REG_CODE << 4)) != types)
	{
		...
		//fprint("types = %d => %d cases %d %d\n", types, types >> 6, (DIR_CODE << 6), (IND_CODE << 6));
		return (1);
	}	
	return (0);
}

static void	log_lld(short carriage_id, int arg1, short reg_num)
{
	if (!DEBUG_LOG)
		return ;
	fprint("P ");
...
**	the carry
*/
/// Modify carry!!! It was written in the subject
...
void		operation_lld(t_car *carriage, t_vm *vm)
{
	...
	if (!vm || !carriage)
		error_exit(vm, "operation lld - empty ptr found");
	// fprint("operation lld\tcycle %d\tposition %d\n", (int)vm->cw->cycle, (int)carriage->position);
	...
	if (types >> 6 == DIR_CODE)
	{
		...
		carriage->reg[reg_num - 1] = get_bytes(vm->arena, \
				calc_long_ind_address(carriage->position, arg1, vm), 4, vm); // 2 bytes in original cw_vm
