...
/*   Updated: 2020/01/01 21:30:39 by bsabre-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	log_add(short carriage_id, int reg1, int reg2, int reg3)
{
	if (!DEBUG_LOG)
		return ;
	fprint("P ");
...
void	operation_add(t_car *carriage, t_vm *vm)
{
	...
	if (!vm || !carriage)
		error_exit(vm, "operation add - empty ptr found");
	// fprint("cycle %d\tposition %d  operation add\t", (int)carriage->position, (int)vm->cw->cycle);
